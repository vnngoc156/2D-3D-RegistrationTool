...
void
PatchedRayCastHelper<TInputImage, TCoordRep>
::Initialise(void)
{
 ...
 *
 *
 * Rest of this code is the actual PatchedRayCastInterpolateImageFunction
 * class
 *
 *
 **************************************************************************/
...
template<class TInputImage, class TCoordRep>
PatchedRayCastInterpolateImageFunction< TInputImage, TCoordRep >
...
::PrintSelf(std::ostream& os, Indent indent) const
{
  ...
  os << indent << "PostTransform: " << m_PostTransform.GetPointer() << std::endl;

}

template<class TInputImage, class TCoordRep>
typename PatchedRayCastInterpolateImageFunction< TInputImage, TCoordRep >
::SizeType 
PatchedRayCastInterpolateImageFunction< TInputImage, TCoordRep >
::GetRadius() const
{
	//Not Implemented but is useful
    SizeType sizeImage = { {0, 0, 0} };
	return sizeImage;
} 
/* -----------------------------------------------------------------------
...
//----------------------------------------------------------------------------
// Fix to handle the image's direction correctly in the lines below
  ...
  OutputPointType transformedPoint =
    m_PostTransform->TransformPoint( point );

  DirectionType direction = transformedFocalPoint - transformedPoint;
  ...
  PatchedRayCastHelper<TInputImage, TCoordRep> ray;
  ray.SetImage( this->m_Image );
  ray.ZeroState();
  ray.Initialise();

  OutputPointType vectorFromOrigin = this->m_Image->GetOrigin().GetVectorFromOrigin();
...
::EvaluateAtContinuousIndex( const ContinuousIndexType& index ) const
{
  ...
  //    std::lock_guard<std::mutex> guard(m_mut);
  //    /*float epsilon = 0.001;
  //    if (fabs(point[1] - 100.0) <= epsilon && fabs(point[2] - 100.0) <= epsilon) {
  ...
  //    //std::cout << point[1] << " " << point[2] << std::endl;
...
::SetInputImage( const InputImageType * ptr )
{
  ...
  if( this->m_Image )
    ...
    const InputImageType::DirectionType imageDirection = this->m_Image->GetDirection();
    const InputImageType::DirectionType imageDirectionInverse = this->m_Image->GetDirection().GetInverse();
...
//	 itk::Matrix<TCoordRep,3,3>( this->m_DirectionFixed.GetInverse()) );
    ...
    const itk::Point<double, 3> origin = this->m_Image->GetOrigin();
    m_PostTransform->SetCenter( origin );
    }
  else
    {
    m_PostTransform = 0;
